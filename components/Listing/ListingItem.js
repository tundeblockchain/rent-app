...
function ListingItem({ propertyID, connected, showReservedListing,location,country, date, price, image, name, owner, isReserved, reservation, removeListing, toggleEditListingModal, toggleReserveListingModal, unreserveListing, publicKey, setCurrentEditListingID }) {
    ...
    const handleReserve = () => {
        if (isReserved) {
            unreserveListing(propertyID)
            return
        }
        setCurrentEditListingID(propertyID);
        toggleReserveListingModal(true, propertyID)
    }
    return (
        <div className="flex flex-col space-y-3 cursor-pointer">
            <div className="relative h-64 w-auto group">
                ...
                {connected && (publicKey == owner) &&(
                    <div className="opacity-0 group-hover:opacity-100 transition-all duration-150 absolute top-4 right-4 flex space-x-2">
                        <PencilIcon onClick={() => toggleEditListingModal(propertyID)} className="w-6 h-6 text-white opacity-80" />
                        <TrashIcon onClick={() => removeListing({propertyID})} className="w-6 h-6 text-white opacity-80" />
            ...
            <div>
                ...
                <p className="text-sm font-light text-gray-800 mt-2">
                    <span className="text-base font-medium">{price} MATIC</span>
                    &nbsp;per night
                </p>
            </div>
        </div>
